...
await sandbox.fs.writeTextFile("hello.txt", "Hello, sandbox world!");
...
const info = await sandbox.fs.stat("hello.txt");
console.log("File size:", info.size, "bytes");

console.log("\n=== Working with Directories ===\n");

// Create a folder
await sandbox.fs.mkdir("my_project");
console.log("Created my_project/ folder");

// Create nested folders (like mkdir -p)
await sandbox.fs.mkdir("my_project/src/components", { recursive: true });
console.log("Created nested folders");

// Write files in the folder
await sandbox.fs.writeTextFile("my_project/README.md", "# My Project\n\nThis is a test project.");
await sandbox.fs.writeTextFile("my_project/src/index.ts", "console.log('Hello!');");

// List folder contents
console.log("\nContents of my_project/:");
for await (const entry of sandbox.fs.readDir("my_project")) {
...
// Copy a file
await sandbox.fs.copyFile("hello.txt", "hello_backup.txt");
console.log("Copied hello.txt to hello_backup.txt");

// Rename/move a file
await sandbox.fs.rename("hello_backup.txt", "my_project/hello_backup.txt");
console.log("Moved backup into my_project/");

console.log("\n=== Cleanup ===\n");

// Delete a single file
await sandbox.fs.remove("hello.txt");
console.log("Deleted hello.txt");

// Delete a folder and everything inside it
await sandbox.fs.remove("my_project", { recursive: true });
